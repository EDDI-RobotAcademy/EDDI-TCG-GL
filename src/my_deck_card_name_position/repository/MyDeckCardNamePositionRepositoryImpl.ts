...
export class MyDeckCardNamePositionRepositoryImpl implements MyDeckCardNamePositionRepository {
    ...
    private positionMap: Map< number, {cardId: number, position: MyDeckCardNamePosition}> = new Map(); // position ID: {card Id, position}
    ...
    public addMyDeckCardNamePosition(deckId: number, cardId: number): MyDeckCardNamePosition {
        console.log(`%c [Card Name] deckID?: ${deckId}, cardId?: ${cardId}`, 'color: #FE2EF7; font-weight: bold;');
    ...
    public deleteById(deckId: number, positionId: number): void {
        ...
        for (const id of updatedPositionIdList) {
            ...
            const newPosition = new MyDeckCardNamePosition(newX, newY);
