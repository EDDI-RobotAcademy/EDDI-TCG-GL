...
export class MyDeckNumberOfCardsPositionRepositoryImpl implements MyDeckNumberOfCardsPositionRepository {
    ...
    // 검색용 position
    public findSearchNumberPosition(deckId: number, searchResultCount: number): MyDeckNumberOfCardsPosition[] {
        const positionIdList = this.deckToPositionMap.get(deckId) || [];

        // 검색 결과 개수만큼 positionId만 추출
        const limitedPositionIdList = positionIdList.slice(0, searchResultCount);

        // 각 positionId에 해당하는 MyDeckNumberOfCardsPosition 가져오기
        const positions: MyDeckNumberOfCardsPosition[] = [];
