...
#### 5-1-1. 일반 공격과 스킬은 갈리는 축이 다르다

카드가 하는 일을 하나의 정의로 묶으려 하면 안 된다. 둘이 다른 축으로 갈린다.

```
일반 공격    병종 → 무기 → 모션
             병종에 따라 무기가 다르고, 무기에 따라 공격 모션이 바뀐다
             가짓수가 닫혀 있다. 검, 지팡이 같은 무기 종류만큼이다
             카드마다 정의를 쓸 필요가 없다

스킬 1, 2    카드마다 고유하다
             가짓수가 열려 있다. 카드 100장 각각이 다를 수 있다
             카드마다 정의가 필요하다
```

지금 코드가 이미 그렇게 되어 있다. `AttackAnimationV2` 는 카드에 붙은 무기 슬롯이
`sword` 인지 `staff` 인지 보고 모션을 정한다. 카드 번호를 보지 않는다.
반면 스킬 배선(`battle/active_panel/effect/`)은 카드 번호로 분기한다.

**무기 슬롯을 보는 것이 곧 병종을 보는 것이다.** 병종 필드를 따로 읽을 필요가 없다.
전사와 암살자가 둘 다 검을 들면 같은 공격 모션이 맞다.

그래서 카드 능력 정의는 이렇게 갈린다.

| | 카드 데이터에 정의를 쓰는가 |
|---|---|
| 일반 공격 | **쓰지 않는다.** 무기가 정해지면 모션이 정해진다 |
| 스킬 1, 2 | 쓴다. `targeting`, `effect`, 필요하면 `special` |
| 패시브 1, 2 | 쓴다 |

이것을 모르고 공격까지 카드마다 정의하게 만들면, 100장에 같은 내용을 반복해서 쓰게 된다.
