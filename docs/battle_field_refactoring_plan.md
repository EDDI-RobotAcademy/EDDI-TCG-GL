...
**작업은 티켓 단위로 진행한다.**
[티켓 목록 (R2-1~9)](refactoring/INDEX.md) · [백로그 작성 규칙](refactoring/RULES.md) ·
...
- [ ] `dependency-cruiser` 도입 + `npm run depcruise` 스크립트
- [ ] 규칙 1 — `battle/domain/**` → `three` 금지
- [ ] 규칙 2 — `platform/**` → `battle|deck|collection|shop|lobby` 금지
- [ ] 규칙 3 — 컨텍스트 간 직접 import 금지 (`battle/**` → `deck/**` 등)
- [ ] 규칙 4 — `battle/domain/**`의 `Math.random()` 금지 (결정론 보장)
- [ ] 테스트가 없는 상태에서 구조 위반을 잡을 **유일한 자동 수단**이므로
      폴더를 옮기기 전에 먼저 넣는다
