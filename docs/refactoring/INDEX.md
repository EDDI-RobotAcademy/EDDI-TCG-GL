...
| **R2-149** | Strategy | Platform | 새 구조 껍데기 셋을 살릴지 지울지 정한다 | R2-146 | **대기.** `src/ui/screens/` `src/game/` `src/input/` 합쳐 280줄이 배경 한 장만 그린다. 덱 작업자가 9개월 전에 시작해 둔 것이다. 지금 지우면 [기준으로 쓸 수 있었는데] 가 되고, 살리면 [미완성에 왜 투자하지] 가 된다 |
