...
| **R2-131** | Structure | Platform | 로비 화면의 구현 방식을 전투와 통일한다 | R2-133 | **대기.** 조건 — 덱을 앱에 붙이기 직전. 지금 하면 덱에서 실제로 필요한 것이 나와 다시 고치게 된다. 값이 로비가 아니라 [다음 화면이 따라갈 기준] 에 있으므로 시점도 다음 대형 화면 직전이다 |
| **R2-132** | Strategy | Deck | 덱 화면 두 벌 중 살릴 쪽을 정한다 | R2-130 | 예정. **draw-my-deck 과 refactor-draw-my-deck 둘 다 띄워 무엇이 도는지 잰다. 고르면 48,529줄의 절반이 빠진다** |
| **R2-133** | Structure | Deck | 덱 화면을 앱에서 쓸 수 있게 한다 | R2-132 | 예정. **가장 크다. 보유 카드 판을 이 화면 안에 넣고, 내 카드 단추를 뺀다. 카드 목록·거르기·찾기는 나중에 도감이 그대로 쓸 수 있게 둔다** |
| **R2-134** | Structure | Platform | 레이드 자리를 만들어 메뉴 다섯을 갖춘다 | R2-130 | 예정. **로비 ─ 덱 ─ 전투 ─ 상점 ─ 레이드. 레이드는 빈 화면이다. 전투에서 만든 방식으로 만든다** |
| **R2-135** | Rule | Shop | 상점에서 카드를 실제로 살 수 있게 한다 | R2-133 | 예정. **지금은 그림과 단추만 있다** |
| **R2-136** | Rendering | Collection | 안 가진 카드까지 보여 주는 도감을 만든다 | R2-133 | 예정. **출시 뒤. 덱의 카드 목록 부품을 그대로 쓴다. 상점이 돌기 전에는 회색 카드만 보여 주고 갈 데가 없다** |
| **R2-137** | Strategy | Battle Ability | 새 카드 열 장으로 확장 시험 | R2-129 | 예정. **여기부터는 리팩토링이 아니다** |
...
| **R2-141** | Rendering | Battle Ability | 광역기 연출을 카드별로 분리 | R2-124 | 예정. **카드가 내놓는 것에 광역기 연출 자리를 먼저 더해야 한다. 네더 블레이드는 신화 등급 연출이 아직 없다** |
