...
| **R2-112** | Structure | Battle | 판을 차리는 일과 전투를 셈하는 일을 화면 밖으로 뺀다 | R2-111 | 예정. **둘은 성격이 다르다. 판 차리기는 확인용 화면의 시작 상태 만들기, 셈하기는 실행 권한. 본문에서 갈라 적을 것** |
| **R2-113** | Rendering | Battle 공용 | 화면이 직접 그리던 다섯 덩어리를 렌더러가 맡게 한다 | R2-107 | 예정 |
| **R2-114** | Structure | Battle Field | 필드 영역이 그림을 들지 않게 한다 | R2-107 | 예정 |
| **R2-115** | Structure | Battle | 전투 화면을 맡은 일별로 가른다 | R2-113 | 예정 |
| **R2-116** | Structure | Battle Ability | 신규 카드 추가 시 전투 화면을 고치지 않도록 연출을 카드별로 분리 | R2-115 | 예정. **R2-111 의 나머지 절반. 화면이 한 덩어리라 R2-115 뒤에 온다** |
| **R2-117** | Strategy | Battle Ability | 새 카드 열 장으로 확장 시험 | R2-116 | 예정. **여기부터는 리팩토링이 아니다** |
| **R2-118** | Rendering | Battle 공용 | 체력과 무기 표기를 그림에서 글자로 바꾼다 | R2-46 | 예정. **숫자 없는 바탕 그림 셋이 있어야 한다** |
| **R2-119** | Rule | Battle | 멀리건을 더해 1대1 대전이 되게 한다 | R2-112 | 예정 |
