...
| **R2-127** | Structure | Battle | 사람이 봐야 했던 경계 셋을 기계가 검사하게 한다 | R2-126 | 예정. **지금 위반이 0곳이라 붙일 수 있다. 늘어난 뒤에는 못 붙인다** |
| **R2-128** | Structure | Battle | 남겨 둔 자리 셋에 무엇을 위한 것인지 적는다 | R2-126 | 예정. **지운 자리에 남은 찌꺼기로 읽힌다** |
| **R2-129** | Structure | Battle Unit | 때리기가 기대는 것 서른하나를 셋으로 가른다 | R2-124 | 예정. **패널, 대상 고르기, 맞은 결과 그리기 셋이 섞여 있다** |
...
| **R2-131** | Strategy | Battle Ability | 새 카드 열 장으로 확장 시험 | R2-129 | 예정. **여기부터는 리팩토링이 아니다** |
| **R2-132** | Rendering | Battle 공용 | 체력과 무기 표기를 그림에서 글자로 바꾼다 | R2-46 | 예정. **숫자 없는 바탕 그림 셋이 있어야 한다** |
| **R2-133** | Rule | Battle | 멀리건을 더해 1대1 대전이 되게 한다 | R2-126 | 예정 |
| **R2-134** | Structure | Platform | 사용자에게 내보낼 것을 만드는 명령을 만든다 | R2-82 | 예정 |
| **R2-135** | Rendering | Battle Ability | 광역기 연출을 카드별로 분리 | R2-124 | 예정. **카드가 내놓는 것에 광역기 연출 자리를 먼저 더해야 한다. 네더 블레이드는 신화 등급 연출이 아직 없다** |
