...
| **R2-136** | Strategy | Battle Ability | 새 카드 열 장으로 확장 시험 | R2-129 | 예정. **다섯 덩어리가 제대로 나뉘었는지 재는 시험이다** |
| **R2-137** | Rendering | Battle 공용 | 체력과 무기 표기를 그림에서 글자로 바꾼다 | R2-46 | 예정. **숫자 없는 바탕 그림 셋이 있어야 한다** |
| **R2-138** | Rule | Battle | 멀리건을 더해 1대1 대전이 되게 한다 | R2-126 | 예정 |
| **R2-139** | Structure | Platform | 사용자에게 내보낼 것을 만드는 명령을 만든다 | R2-82 | 예정 |
| **R2-140** | Rendering | Battle Ability | 광역기 연출을 카드별로 분리 | R2-124 | 예정. **카드가 내놓는 것에 광역기 연출 자리를 먼저 더해야 한다** |

### 덱 — 그쪽 작업이 올라오기 전까지 진행 불가

덱 작업이 다른 사람 기계에만 있고 아직 커밋조차 안 됐다. 우리 쪽에서 찾을 방법이 없다.
**「완성해서 PR 주세요」가 아니라 「지금 상태 그대로 커밋해서 올려 주세요」** 를 요청한다 —
동작 안 해도 된다. 올라오기만 하면 R2-142 를 시작할 수 있다.
...
| 번호 | Type | Domain | 제목 | 선행 | 상태 |
|---|---|---|---|---|---|
| **R2-141** | Rule | Battle Deck | 짠 덱을 전투가 쓰게 한다 | R2-126 | **대기.** 조건 — 덱 화면이 무엇을 내놓는지 정해진 뒤. 지금 전투는 파일에 적어 둔 카드 열두 장을 쓴다 |
| **R2-142** | Strategy | Deck | 덱 기능의 현재 상태와 완성 조건을 확정한다 | — | **대기.** 조건 — 덱 작업자가 지금 상태 그대로 커밋해 올림. 열 갈래(덱 생성·보유 카드 조회·카드 추가·카드 제거·거르기·찾기·편집·완료·저장 복구·전투로 넘기기) 마다 현재 구현·실제 동작·필요 데이터·출시 필요 여부를 확인한다 |
| **R2-143** | Rule | Deck | 덱 화면을 새로 만든다 | R2-142 | **대기.** 조건 — R2-142 에서 명세가 확정된 뒤. 전면 개편이므로 처음부터 새 방식으로 만든다. 받은 것은 코드가 아니라 **명세** 로 쓴다 |
| **R2-144** | Structure | Deck | 내 카드 화면을 덱 안으로 넣는다 | R2-143 | **대기.** 보유 카드를 훑는 일이 덱 화면의 일부다. 메뉴에서 [내 카드] 를 빼고, 카드 한 장 크게 보기만 덱으로 옮긴다 |
| **R2-145** | Rendering | Collection | 안 가진 카드까지 보여 주는 도감을 만든다 | R2-143 | **대기.** 출시 뒤. 덱의 카드 목록·거르기·찾기 부품을 그대로 쓴다 |
| **R2-146** | Strategy | Platform | 새 구조 껍데기 셋을 살릴지 지울지 정한다 | R2-143 | **대기.** `src/ui/screens/` `src/game/` `src/input/` 합쳐 280줄이 배경 한 장만 그린다. 덱 작업자가 9개월 전에 시작해 둔 것이다. 지금 지우면 [기준으로 쓸 수 있었는데] 가 되고, 살리면 [미완성에 왜 투자하지] 가 된다 |
