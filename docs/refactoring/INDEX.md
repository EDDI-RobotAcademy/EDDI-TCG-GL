...
| **R2-133** | Structure | Shop | [상점 화면의 구현 방식을 전투와 통일한다](R2-133-shop-same-shape.md) | R2-132 | **완료. 여섯 군데가 한 곳으로. 옮기면서 버그 셋을 고쳤다** |
| **R2-134** | Structure | Platform | 화면을 더할 때마다 순환 참조가 늘어나는 것을 끊는다 | R2-132 | 예정. **화면이 길 안내를 들고 길 목록이 화면을 들여온다. 화면 다섯에 경고 다섯. 화면 수에 비례해 늘어난다** |
| **R2-135** | Rule | Battle Deck | 짠 덱을 전투가 쓰게 한다 | R2-126 | 예정. **지금 전투는 파일에 적어 둔 카드 열두 장을 쓴다. 덱과 전투가 아직 안 이어져 있다. 덱 작업자 PR 과 무관하게 우리 쪽에서 할 수 있다** |
| **R2-136** | Rule | Shop | 상점에서 카드를 실제로 살 수 있게 한다 | R2-130 | 예정. **지금은 그림과 단추만 있다** |
| **R2-137** | Strategy | Deck | 덱 기능의 현재 상태와 완성 조건을 확정한다 | — | **대기.** 조건 — 덱 작업자 PR 수신. 지금 되는 것은 편집 일부뿐이고 덱 만들기·카드 넣기는 안 된다. 저장도 없다. 열 갈래(덱 생성·보유 카드 조회·카드 추가·카드 제거·거르기·찾기·편집·완료·저장 복구·전투로 넘기기) 마다 현재 구현·실제 동작·필요 데이터·출시 필요 여부를 확인한다 |
| **R2-138** | Rule | Deck | 덱 만들기·편집·카드 넣기를 완성한다 | R2-137 | **대기.** 조건 — R2-137 에서 완성 조건이 확정된 뒤. 이것 없이는 전투에 들고 갈 카드가 없다 |
| **R2-139** | Strategy | Battle Ability | 새 카드 열 장으로 확장 시험 | R2-129 | 예정. **다섯 덩어리가 제대로 나뉘었는지 재는 시험이다** |
| **R2-140** | Rendering | Battle 공용 | 체력과 무기 표기를 그림에서 글자로 바꾼다 | R2-46 | 예정. **숫자 없는 바탕 그림 셋이 있어야 한다** |
| **R2-141** | Rule | Battle | 멀리건을 더해 1대1 대전이 되게 한다 | R2-126 | 예정 |
| **R2-142** | Structure | Platform | 사용자에게 내보낼 것을 만드는 명령을 만든다 | R2-82 | 예정 |
| **R2-143** | Rendering | Battle Ability | 광역기 연출을 카드별로 분리 | R2-124 | 예정. **카드가 내놓는 것에 광역기 연출 자리를 먼저 더해야 한다** |
| **R2-144** | Rendering | Collection | 안 가진 카드까지 보여 주는 도감을 만든다 | R2-138 | 예정. **출시 뒤. 덱의 카드 목록 부품을 그대로 쓴다** |
| **R2-145** | Structure | Deck | 덱 화면의 구현 방식을 전투와 통일한다 | R2-138 | **대기.** 조건 — 덱 기능이 완성된 뒤. 미완성인 것을 다듬으면 헛일이 된다 |
| **R2-146** | Strategy | Platform | 새 구조 껍데기 셋을 살릴지 지울지 정한다 | R2-138 | **대기.** 조건 — 덱 기능 완성 뒤 재평가. `src/ui/screens/` `src/game/` `src/input/` 합쳐 280줄이 배경 한 장만 그린다. 지금 지우면 [기준으로 쓸 수 있었는데] 가 되고, 살리면 [미완성에 왜 투자하지] 가 된다. 정보가 부족할 때 구조를 확정하지 않는다 |
