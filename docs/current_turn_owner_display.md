...
1. **상시 턴 인디케이터**
   - [ ] 배너는 전환 순간만 알리고 3초 뒤 사라지므로 "지금 누구 턴인지"를 상시 확인할
         방법이 없음. 턴 HUD에 `YOUR / OPPONENT` 뱃지 추가 검토
   - [ ] 상대 턴 동안 턴 종료 버튼을 비활성 상태(디밍/채도 저하)로 표현 — 현재는 눌러도
         무반응일 뿐 눌러도 되는 것처럼 보임
   - [ ] 상대 턴 동안 손패 잠금 표현 추가 — `canPickup`이 false를 반환해 카드가 들리지
         않지만 이유가 화면에 드러나지 않음

2. **턴 전환 트리거를 실제 로직으로 대체**
   - [ ] `f` 키는 임시 디버그 조작 — 상대 턴 로직(AI 또는 네트워크 동기화)으로 대체 필요.
         현재는 상대가 아무 행동도 하지 않고 키 입력만 기다림
   - [ ] 모래시계 60초 만료 시 자동 턴 넘김 없음. `startLoop`가 0에서 rAF를 멈추기만 하고
         만료 콜백이 없으므로 `onExpire` 훅 추가 후 턴 종료와 연결
   - [ ] 키 판정을 `e.key` → `e.code`(`'KeyF'`, `'KeyD'`)로 변경 검토 — 한글 IME 상태에서는
         `e.key`가 `'ㄹ'`로 들어와 핸들러가 통과되지 않음

3. **메인 앱 반영**
   - [ ] `src/ui/screens/battle_field/BattleFieldView.ts`에는 미반영.
         `LeftClickDetectServiceImpl.handleTurnEndClick()`이 빈 스텁이라 턴 상태 전환 자체가
         없음. 턴 상태 연결이 선행되어야 함
   - [ ] `BattleFieldView`가 아직 구버전 전역 함수 `showGuideMessage()`를 사용 —
         `GuideMessageHudRendererV2`로 교체

4. **연출 / 타이밍**
   - [ ] `f` 키 처리에서 배너가 카드 드로우 애니메이션보다 먼저 떠 서로 겹침 —
         드로우 완료 후 표시하거나 지속시간을 드로우 길이에 맞추는 방안 검토
   - [ ] 배너에 페이드 인/아웃 트랜지션 부재 — 현재 `display` 토글로 즉시 나타났다 즉시 사라짐
   - [ ] 내 턴 / 상대 턴 배너의 색상 구분 검토 (현재 둘 다 청록 `#00ffd0`)
   - [ ] 턴 전환 효과음 (내 턴 시작 / 상대 턴 시작 구분)

5. **전환 부수효과 통합**
   - [ ] 배너·타이머 리셋·TURN 증가·에너지 증가·드로우가 클릭 핸들러와 키 핸들러에 흩어져
         있음 — `TurnTransitionHandler`로 묶어 두 진입점이 같은 함수를 호출하도록 정리
   - [ ] `currentTurn`을 `TurnStateRepository`가 소유권과 함께 보유하도록 승격
         (출격 멀미 백로그와 공통 항목)
   - [ ] 문구 `'상대방의 턴입니다.'` / `'당신의 턴입니다.'` 하드코딩 — 메시지 상수/i18n 분리
